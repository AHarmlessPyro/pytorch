...
#ifdef NATIVE_FP16
  float fp32 = (float) reinterpret_cast<_Float16>(h);
  return reinterpret_cast<uint32_t>(fp32);
#else // !NATIVE_FP16
  /*
...
C10_HOST_DEVICE inline float fp16_ieee_to_fp32_value(uint16_t h) {
  ...
  return (float) reinterpret_cast<_Float16>(h);
#elif defined(X86_F16)
  return _cvtsh_ss(h);
#else // ! NATIVE_FP16 && ! X86_F16
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
...
#if defined(NATIVE_FP16)
  _Float16 fp16 = static_cast<_Float16>(f);
  return reinterpret_cast<uint16_t>(fp16);
#elif defined(X86_F16)
  return _cvtss_sh(f, _MM_FROUND_TO_NEAREST_INT);
#else // !defined(NATIVE_FP16) && !defined(X86_F16)
